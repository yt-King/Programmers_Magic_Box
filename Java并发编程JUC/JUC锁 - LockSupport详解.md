...
- park函数，阻塞线程，并且该线程在下列情况发生之前都会被阻塞: ① 调用unpark函数，释放该线程的许可。② 该线程被中断。③ 设置的时间到了。并且，如果isAbsolute是true则会实现ms定时。如果isAbsolute是false则会实现ns定时。。当time为0时，表示无限等待，直到unpark发生。
- unpark函数，释放线程的许可，即激活调用park后阻塞的线程。这个函数不是安全的，调用这个函数时要确保线程依旧存活。

#### park函数

park函数有两个重载版本，方法摘要如下，两个函数的区别在于park()函数有没有blocker，即有没有设置线程的parkBlocker字段
...
public static void parkNanos(Object blocker, long nanos) {
    if (nanos > 0) { // 时间大于0
        ...
        // 设置Blocker
        setBlocker(t, blocker);
        // 获取许可，并设置了时间，时间到了如果未收到唤醒信号也会返回继续执行
...
public static void parkUntil(Object blocker, long deadline) {
    ...
    // 设置Blocker
    setBlocker(t, blocker);
    //如果time <= 0则直接返回,如果time > 0,则会挂起当前线程，但是在时间到了如果未收到唤醒信号也会返回继续执行
...
流程如图：![image-20220804142904346](https://typora-imagehost-1308499275.cos.ap-shanghai.myqcloud.com/2022-8/202208041429402.png)
...
Object.wait()和Condition.await()的原理是基本一致的，不同的是**Condition.await()底层是调用LockSupport.park()来实现阻塞当前线程的。**
...
不会，它**只负责阻塞当前线程**，**释放锁资源实际上是在Condition的await()方法中实现的**
